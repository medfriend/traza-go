...
func (o *Observer) Start(ctx context.Context) error {
	...
	_, err := o.ch.QueueDeclare(
		o.queueName, // nombre de la cola
		true,        // durable (la cola persiste en RabbitMQ)
		false,       // delete when unused (auto-eliminación)
		false,       // exclusive (solo puede ser usada por este consumidor)
		false,       // no-wait (espera a que RabbitMQ confirme)
		nil,         // argumentos adicionales
	)
	if err != nil {
		return fmt.Errorf("error al declarar la cola %s: %w", o.queueName, err)
	}
